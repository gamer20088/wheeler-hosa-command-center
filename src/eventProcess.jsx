import { CalendarCheck, CheckCircle2, ClipboardCheck, ClipboardList, Search } from 'lucide-react'
...
function EventPreviewPanel({ event, setSelectedEventId, setTab, useEvent }) {
  if (!event) {
    return <MiniCard className="xl:sticky xl:top-5"><h3 className="text-lg font-black text-slate-950">No event selected</h3><p className="mt-2 text-sm leading-6 text-slate-600">Adjust the filters or search to find an event to preview.</p></MiniCard>
  }
  ...
  return <aside className="xl:sticky xl:top-5 xl:self-start"><MiniCard className="space-y-4"><div><p className="text-xs font-black uppercase tracking-wide text-teal-700">Selected Preview</p><h3 className="mt-1 text-2xl font-black tracking-tight text-slate-950">{safeText(event.name, 'Untitled event')}</h3><p className="mt-1 text-sm font-semibold text-teal-700">{safeText(event.track, 'Verify track with advisor')}</p><ProcessBadges event={event} /></div><div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Why this event fits</p><p className="mt-2 text-sm leading-6 text-slate-700">{safeText(event.why, 'Review this event with the Wheeler advisor.')}</p></div><ProcessDetails event={event} /><ListBlock title="This week's first action" items={[safeText(event.firstAction, 'Review the official guideline and ask the advisor what to verify first.')]} icon={CalendarCheck} /><ListBlock title="Evidence to collect" items={safeArray(event.evidence).slice(0, 5)} icon={ClipboardCheck} /><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1"><MiniButton onClick={() => useEvent(event)} className="w-full">Use this event</MiniButton><MiniButton onClick={openPlan} variant="light" className="w-full">Open My Plan</MiniButton></div></MiniCard></aside>
}
...
export function EnhancedEventLibrary({ events, selectedEventId, setSelectedEventId, setTab }) {
  ...
  const [activePreviewId, setActivePreviewId] = useState(selectedEventId || null)
  const normalizedEvents = normalizeEvents(events)
  ...
  const activePreviewEvent = filteredEvents.find((event) => event.id === activePreviewId) || filteredEvents[0] || null
  ...
  return <div className="space-y-5"><MiniCard><div className="grid gap-4 lg:grid-cols-[1fr_2fr] lg:items-end"><div className="flex gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-100 text-teal-700"><ClipboardList size={21} /></div><div><h2 className="text-xl font-black tracking-tight text-slate-950 sm:text-2xl">Event Library</h2><p className="mt-1 text-sm leading-6 text-slate-500">Browse events, then preview details in one clean panel.</p><p className="mt-1 text-xs font-black uppercase tracking-wide text-teal-700">Showing {filteredEvents.length} of {normalizedEvents.length} events</p></div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><label className="block sm:col-span-2 xl:col-span-1"><span className="mb-1 block text-xs font-black uppercase tracking-wide text-slate-600">Search</span><div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input aria-label="Search events" value={filters.query} onChange={(event) => setFilters({ ...filters, query: event.target.value })} placeholder="Search" className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none transition hover:border-teal-300 focus:ring-2 focus:ring-teal-300" /></div></label><Select label="Track" value={filters.track} options={tracks} onChange={(track) => setFilters({ ...filters, track })} /><Select label="Difficulty" value={filters.difficulty} options={difficulties} onChange={(difficulty) => setFilters({ ...filters, difficulty })} /><Select label="Format" value={filters.team} options={['All', 'Solo', 'Team event']} onChange={(team) => setFilters({ ...filters, team })} /><Select label="Commitment" value={filters.commitment} options={['All', 'Low lift', 'Serious', 'High commitment', 'Only for locked-in teams']} onChange={(commitment) => setFilters({ ...filters, commitment })} /><Select label="Process" value={filters.process} options={['All', 'Test Event', 'Pre-Judged Upload', 'Presentation Event', 'Recognition Exam', 'No Upload', 'Verify SLC Rules']} onChange={(process) => setFilters({ ...filters, process })} /></div></div><div className="mt-4"><WarningBox /></div></MiniCard><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.45fr)]"><div className="grid gap-4 md:grid-cols-2">{filteredEvents.map((event) => { const active = activePreviewEvent?.id === event.id; return <button key={event.id} type="button" onClick={() => setActivePreviewId(event.id)} className={cx('cursor-pointer rounded-2xl bg-white p-4 text-left shadow-sm ring-1 ring-slate-200 transition hover:bg-teal-50 hover:ring-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-300', active && 'bg-teal-50 ring-2 ring-teal-400')}><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-black text-slate-950">{safeText(event.name, 'Untitled event')}</h3><p className="mt-1 text-sm font-semibold text-teal-700">{safeText(event.track, 'Verify track with advisor')}</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black text-slate-700">{event.time || 4}+ hrs</span></div><ProcessBadges event={event} max={4} /><div className="mt-3 grid grid-cols-2 gap-2 text-xs font-bold text-slate-600"><span className="rounded-lg bg-slate-50 p-2 ring-1 ring-slate-200">{safeText(event.difficulty, 'Verify')}</span><span className="rounded-lg bg-slate-50 p-2 ring-1 ring-slate-200">{event.team === 'Solo' ? 'Solo' : 'Team'}</span></div><p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-700">{safeText(event.why, 'Review this event with the Wheeler advisor.')}</p><span className={cx('mt-4 inline-flex min-h-10 w-full items-center justify-center rounded-xl px-3 py-2 text-sm font-black transition', active ? 'bg-slate-950 text-white' : 'bg-white text-slate-950 ring-1 ring-slate-200')}>{active ? 'Selected for preview' : 'View event'}</span></button> })}</div><EventPreviewPanel event={activePreviewEvent} setSelectedEventId={setSelectedEventId} setTab={setTab} useEvent={useEvent} /></div></div>
}
