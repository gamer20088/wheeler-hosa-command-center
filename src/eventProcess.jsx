...
function CompareShortlistModal({ events, onClose, useEvent }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" onClick={onClose}><section role="dialog" aria-modal="true" aria-labelledby="compare-shortlist-title" className="max-h-[90vh] w-full max-w-6xl overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl ring-1 ring-slate-200 sm:p-6" onClick={(clickEvent) => clickEvent.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wide text-blue-950">Shortlist Compare</p><h3 id="compare-shortlist-title" className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Compare saved events</h3><p className="mt-1 text-sm font-semibold leading-6 text-slate-600">Pick the event that feels realistic, clear, and worth practicing every week.</p></div><button type="button" aria-label="Close shortlist compare" onClick={onClose} className="inline-flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-slate-100 text-slate-700 transition hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-300"><X size={18} /></button></div><div className="mt-5 grid gap-3 lg:grid-cols-2 xl:grid-cols-4">{safeArray(events).map((event) => {
...
export function EnhancedEventLibrary({ events, setSelectedEventId, setTab }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Shortlist is a local convenience only; the library should still work if storage is blocked.
    }
  ...
  const toggleShortlist = (event) => {
    ...
    if (shortlistIds.length >= SHORTLIST_LIMIT) {
      setShortlistMessage('Shortlist is full. Remove one event before adding another.')
      return
    }
