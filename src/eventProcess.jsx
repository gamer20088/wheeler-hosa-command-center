...
const SHORTLIST_STORAGE_KEY = 'wheelerHosaShortlist'
...
function MiniButton({ children, onClick, variant = 'primary', icon: Icon, className = '' }) {
  return <button onClick={onClick} className={cx('inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition focus:outline-none focus:ring-2 focus:ring-blue-300', variant === 'primary' && 'bg-blue-950 text-white hover:bg-blue-900', variant === 'light' && 'bg-white text-slate-900 ring-1 ring-slate-200 hover:bg-blue-50 hover:ring-blue-300', className)}>{Icon && <Icon size={16} />}{children}</button>
}

function Select({ label, value, options, onChange }) {
  return <label className="block"><span className="mb-1 block text-xs font-black uppercase tracking-wide text-slate-600">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="h-11 w-full cursor-pointer rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition hover:border-blue-300 hover:bg-blue-50/40 focus:ring-2 focus:ring-blue-300">{options.map((option) => <option key={option}>{option}</option>)}</select></label>
}
...
function EventOverviewModal({ event, onClose, useEvent }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" onClick={onClose}><section role="dialog" aria-modal="true" aria-labelledby="event-overview-title" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl ring-1 ring-slate-200 sm:p-6" onClick={(clickEvent) => clickEvent.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wide text-blue-950">Event Overview</p><h3 id="event-overview-title" className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{safeText(event.name, 'Untitled event')}</h3><p className="mt-1 text-sm font-semibold text-blue-950">{safeText(event.track, 'Verify track with advisor')}</p><ProcessBadges event={event} /></div><button type="button" aria-label="Close event preview" onClick={onClose} className="inline-flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-slate-100 text-slate-700 transition hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-300"><X size={18} /></button></div><div className="mt-5 space-y-4"><div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Why this event fits</p><p className="mt-2 text-sm leading-6 text-slate-700">{safeText(event.why, 'Review this event with the Wheeler advisor.')}</p></div><ProcessDetails event={event} /><ListBlock title="This week's first action" items={[safeText(event.firstAction, 'Review the official guideline and ask the advisor what to verify first.')]} icon={CalendarCheck} /><ListBlock title="Evidence to collect" items={safeArray(event.evidence).slice(0, 5)} icon={ClipboardCheck} /><MiniButton onClick={chooseEvent} className="w-full">Use this event</MiniButton></div></section></div>
}

function CompareShortlistModal({ events, onClose, useEvent }) {
  useEffect(() => {
    const handleKeyDown = (keyboardEvent) => {
      if (keyboardEvent.key === 'Escape') onClose()
    }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" onClick={onClose}><section role="dialog" aria-modal="true" aria-labelledby="compare-shortlist-title" className="max-h-[90vh] w-full max-w-6xl overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl ring-1 ring-slate-200 sm:p-6" onClick={(clickEvent) => clickEvent.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wide text-blue-950">Saved Events Compare</p><h3 id="compare-shortlist-title" className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Compare saved events</h3><p className="mt-1 text-sm font-semibold leading-6 text-slate-600">Pick the event that feels realistic, clear, and worth practicing every week.</p></div><button type="button" aria-label="Close saved events compare" onClick={onClose} className="inline-flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-slate-100 text-slate-700 transition hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-300"><X size={18} /></button></div><div className="mt-5 grid gap-3 lg:grid-cols-2 xl:grid-cols-4">{safeArray(events).map((event) => {
    const evidenceItems = safeArray(event.evidence).length ? safeArray(event.evidence) : getProofExamples(event)
    return <article key={event.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><h4 className="text-lg font-black text-slate-950">{safeText(event.name, 'Untitled event')}</h4><p className="mt-1 text-sm font-bold text-blue-950">{safeText(event.track, 'Verify track with advisor')}</p><ProcessBadges event={event} max={3} /><div className="mt-3 grid gap-2 text-sm leading-6 text-slate-700"><p><span className="font-black text-slate-900">Difficulty:</span> {safeText(event.difficulty, 'Verify')}</p><p><span className="font-black text-slate-900">Weekly time:</span> {event.time || 4}+ hrs/week</p><p><span className="font-black text-slate-900">Format:</span> {event.team === 'Solo' ? 'Solo' : 'Team'}</p><p><span className="font-black text-slate-900">First action:</span> {safeText(event.firstAction, 'Review the guideline and verify next steps.')}</p></div><div className="mt-3"><ListBlock title="Evidence examples" items={evidenceItems.slice(0, 3)} icon={ClipboardCheck} /></div><MiniButton onClick={() => chooseEvent(event)} className="mt-4 w-full">Use this event</MiniButton></article>
  })}</div></section></div>
}
...
export function StudentFriendlyGuideSection({ event }) {
  ...
  return <div className="rounded-xl bg-white p-4 ring-1 ring-slate-200"><h3 className="text-sm font-black text-slate-950">What You Are Signing Up For</h3><div className="mt-4 grid gap-4"><GuideBlock title="In baby words"><p className="text-sm leading-6 text-slate-700">{guide.babySummary}</p></GuideBlock><GuideBlock title="What you actually do"><GuideBulletList items={guide.whatYouActuallyDo} /></GuideBlock><GuideBlock title="Requirements you must handle"><GuideBulletList items={guide.requirementsChecklist} /></GuideBlock><GuideBlock title="Before Georgia SLC"><GuideBulletList items={guide.beforeSlcChecklist} /></GuideBlock><GuideBlock title="Do this first"><p className="rounded-xl bg-blue-50 p-3 text-sm font-bold leading-6 text-blue-950 ring-1 ring-blue-100">{guide.firstPracticeTask}</p></GuideBlock><GuideBlock title="How you know you are ready"><GuideBulletList items={guide.readinessSigns} /></GuideBlock><GuideBlock title="Watch out"><p className="text-sm font-bold leading-6 text-rose-800">{guide.commonTrap}</p></GuideBlock></div></div>
}

export function CompactGuideCard({ event, dark = false }) {
  const guide = getStudentFriendlyGuide(event)
  return <div className={cx('mt-3 rounded-xl p-3 text-sm leading-6 ring-1', dark ? 'bg-white/10 text-slate-100 ring-white/15' : 'bg-white text-slate-700 ring-slate-200')}><p className={cx('text-xs font-black uppercase tracking-wide', dark ? 'text-blue-200' : 'text-blue-950')}>In baby words</p><p className="mt-1">{guide.babySummary}</p><div className="mt-3 grid gap-2"><p><span className="font-black">Main work:</span> {safeText(guide.whatYouActuallyDo[0], 'Practice the event format.')}</p><p><span className="font-black">Main requirement:</span> {safeText(guide.requirementsChecklist[0], 'Verify the rules with the advisor.')}</p><p><span className="font-black">Do this first:</span> {guide.firstPracticeTask}</p><p><span className="font-black">Watch out:</span> {guide.commonTrap}</p></div></div>
}

export function PlainEnglishPlan({ event }) {
  const guide = getStudentFriendlyGuide(event)
  return <CardLike><h3 className="text-base font-black text-slate-950">Your Event in Plain English</h3><p className="mt-3 text-sm leading-6 text-slate-700">{guide.babySummary}</p><div className="mt-4 rounded-xl bg-blue-50 p-3 ring-1 ring-blue-100"><p className="text-xs font-black uppercase tracking-wide text-blue-950">Do This First</p><p className="mt-2 text-sm font-black leading-6 text-blue-950">{guide.firstPracticeTask}</p></div><div className="mt-4 grid gap-4"><GuideBlock title="Requirements"><GuideBulletList items={guide.requirementsChecklist} /></GuideBlock><GuideBlock title="Before Georgia SLC"><GuideBulletList items={guide.beforeSlcChecklist} /></GuideBlock><GuideBlock title="Ready when"><GuideBulletList items={guide.readinessSigns} /></GuideBlock></div><div className="mt-4"><WarningBox /></div></CardLike>
}
...
export function EnhancedEventLibrary({ events, setSelectedEventId, setTab }) {
  ...
  const normalizedEvents = normalizeEvents(events)
  const eventById = new Map(normalizedEvents.map((event) => [safeText(event.id), event]))
  const shortlistEvents = shortlistIds.map((eventId) => eventById.get(eventId)).filter(Boolean)
  ...
  useEffect(() => {
    try {
      const rawShortlist = JSON.parse(localStorage.getItem(SHORTLIST_STORAGE_KEY) || '[]')
      setShortlistIds(safeArray(rawShortlist).map((eventId) => String(eventId)).slice(0, SHORTLIST_LIMIT))
    } catch {
      setShortlistIds([])
    }
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(SHORTLIST_STORAGE_KEY, JSON.stringify(shortlistIds.slice(0, SHORTLIST_LIMIT)))
    } catch {
      // Saved events are a local convenience only; the library should still work if storage is blocked.
    }
  ...
  const toggleShortlist = (event) => {
    ...
    if (!eventId) return
    setShortlistMessage('')
    if (shortlistIds.includes(eventId)) {
      setShortlistIds(shortlistIds.filter((savedId) => savedId !== eventId))
      return
    }
    if (shortlistIds.length >= SHORTLIST_LIMIT) {
      setShortlistMessage('Saved events is full. Remove one event before saving another.')
      return
    }
    setShortlistIds([...shortlistIds, eventId])
  }
  const openCompare = () => {
    if (!shortlistEvents.length) {
      setShortlistMessage('Save events here before choosing your main event.')
      return
    }
    setCompareOpen(true)
  }

  return <div className="space-y-5"><MiniCard><div className="grid gap-4 lg:grid-cols-[1fr_2fr] lg:items-end"><div className="flex gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-950"><ClipboardList size={21} /></div><div><h2 className="text-xl font-black tracking-tight text-slate-950 sm:text-2xl">Event Library</h2><p className="mt-1 text-sm leading-6 text-slate-500">Browse events, then open a quick overview when one looks interesting.</p><p className="mt-1 text-xs font-black uppercase tracking-wide text-blue-950">Showing {filteredEvents.length} of {normalizedEvents.length} events</p></div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><label className="block sm:col-span-2 xl:col-span-1"><span className="mb-1 block text-xs font-black uppercase tracking-wide text-slate-600">Search</span><div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input aria-label="Search events" value={filters.query} onChange={(event) => setFilters({ ...filters, query: event.target.value })} placeholder="Search" className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none transition hover:border-blue-300 focus:ring-2 focus:ring-blue-300" /></div></label><Select label="Track" value={filters.track} options={tracks} onChange={(track) => setFilters({ ...filters, track })} /><Select label="Difficulty" value={filters.difficulty} options={difficulties} onChange={(difficulty) => setFilters({ ...filters, difficulty })} /><Select label="Format" value={filters.team} options={['All', 'Solo', 'Team event']} onChange={(team) => setFilters({ ...filters, team })} /><Select label="Commitment" value={filters.commitment} options={['All', 'Low lift', 'Serious', 'High commitment', 'Only for locked-in teams']} onChange={(commitment) => setFilters({ ...filters, commitment })} /><Select label="Process" value={filters.process} options={['All', 'Test Event', 'Pre-Judged Upload', 'Presentation Event', 'Recognition Exam', 'No Upload', 'Verify SLC Rules']} onChange={(process) => setFilters({ ...filters, process })} /></div></div><div className="mt-4 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-black text-slate-950">Saved Events</h3><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black text-blue-950 ring-1 ring-slate-200">{shortlistEvents.length} of {SHORTLIST_LIMIT}</span><span className="text-xs font-bold text-slate-500">Saved on this device only.</span></div>{shortlistEvents.length ? <div className="mt-2 flex flex-wrap gap-2">{shortlistEvents.map((event) => <span key={event.id} className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-700 ring-1 ring-slate-200">{safeText(event.name, 'Untitled event')}</span>)}</div> : <p className="mt-2 text-sm font-bold leading-6 text-slate-600">Save events here before choosing your main event.</p>}{shortlistMessage && <p className="mt-2 text-sm font-black text-rose-800">{shortlistMessage}</p>}</div><div className="flex flex-col gap-2 sm:flex-row"><MiniButton variant="light" onClick={clearShortlist}>Clear saved events</MiniButton><MiniButton onClick={openCompare} className={!shortlistEvents.length ? 'opacity-60' : ''}>Compare saved events</MiniButton></div></div></div><div className="mt-4"><WarningBox /></div></MiniCard><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filteredEvents.map((event) => {
    const eventId = safeText(event.id)
    const isShortlisted = shortlistIds.includes(eventId)
    return <article key={event.id} role="button" tabIndex={0} onClick={() => setActiveModalEvent(event)} onKeyDown={(keyboardEvent) => { if (keyboardEvent.key === 'Enter' || keyboardEvent.key === ' ') { keyboardEvent.preventDefault(); setActiveModalEvent(event) } }} className={cx('cursor-pointer rounded-2xl bg-white p-3 text-left shadow-sm ring-1 transition hover:bg-blue-50 hover:ring-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-300 sm:p-4', isShortlisted ? 'ring-blue-300' : 'ring-slate-200')}><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-black text-slate-950 sm:text-lg">{safeText(event.name, 'Untitled event')}</h3><p className="mt-0.5 text-sm font-semibold text-blue-950">{safeText(event.track, 'Verify track with advisor')}</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black text-slate-700">{event.time || 4}+ hrs</span></div><ProcessBadges event={event} max={3} /><div className="mt-2 grid grid-cols-2 gap-2 text-xs font-bold text-slate-600"><span className="rounded-lg bg-slate-50 p-2 ring-1 ring-slate-200">{safeText(event.difficulty, 'Verify')}</span><span className="rounded-lg bg-slate-50 p-2 ring-1 ring-slate-200">{event.team === 'Solo' ? 'Solo' : 'Team'}</span></div><p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-700">{safeText(event.why, 'Review this event with the Wheeler advisor.')}</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><button type="button" onClick={(clickEvent) => { clickEvent.stopPropagation(); toggleShortlist(event) }} className={cx('inline-flex min-h-10 cursor-pointer items-center justify-center rounded-xl px-3 py-2 text-sm font-black transition focus:outline-none focus:ring-2 focus:ring-blue-300', isShortlisted ? 'bg-blue-50 text-blue-950 ring-1 ring-blue-200 hover:bg-white' : 'bg-white text-slate-950 ring-1 ring-slate-200 hover:bg-blue-50 hover:ring-blue-300')}>{isShortlisted ? 'Remove saved event' : 'Save event'}</button><button type="button" onClick={(clickEvent) => { clickEvent.stopPropagation(); setActiveModalEvent(event) }} className="inline-flex min-h-10 cursor-pointer items-center justify-center rounded-xl bg-white px-3 py-2 text-sm font-black text-slate-950 ring-1 ring-slate-200 transition hover:bg-blue-950 hover:text-white hover:ring-blue-950 focus:outline-none focus:ring-2 focus:ring-blue-300">View overview</button></div></article>
  })}</div><EventOverviewModal event={activeModalEvent} onClose={() => setActiveModalEvent(null)} useEvent={useEvent} />{compareOpen && <CompareShortlistModal events={shortlistEvents} onClose={() => setCompareOpen(false)} useEvent={useEvent} />}</div>
}
