...
export function PlainEnglishPlan({ event }) {
  const guide = getStudentFriendlyGuide(event)
  return <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-5"><h3 className="text-base font-black text-slate-950">Your Event in Plain English</h3><p className="mt-3 text-sm leading-6 text-slate-700">{guide.babySummary}</p><div className="mt-4 rounded-xl bg-teal-50 p-3 ring-1 ring-teal-100"><p className="text-xs font-black uppercase tracking-wide text-teal-700">Do This First</p><p className="mt-2 text-sm font-black leading-6 text-teal-950">{guide.firstPracticeTask}</p></div><div className="mt-4 grid gap-4"><GuideBlock title="Requirements"><GuideBulletList items={guide.requirementsChecklist} /></GuideBlock><GuideBlock title="Before Georgia SLC"><GuideBulletList items={guide.beforeSlcChecklist} /></GuideBlock><GuideBlock title="Ready when"><GuideBulletList items={guide.readinessSigns} /></GuideBlock></div><div className="mt-4"><StudentGuideWarning /></div></section>
}
