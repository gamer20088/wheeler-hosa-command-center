...
export function ProofOfficerWorkflow() {
  ...
  return <Card><SectionHeader icon={ShieldCheck} title="Officer proof workflow" subtitle="Officers use this tab to review submissions, give feedback, and track readiness." /><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{proofTools.map(([label, url]) => <a key={label} href={url} className="flex min-h-24 cursor-pointer items-center justify-between rounded-2xl bg-slate-50 p-4 text-left font-black text-slate-950 ring-1 ring-slate-200 transition hover:bg-teal-50 hover:ring-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-300">{label}<ExternalLink size={15} className="text-teal-700" /></a>)}</div><div className="mt-4 rounded-xl bg-slate-50 p-3 text-sm font-bold leading-6 text-slate-700 ring-1 ring-slate-200"><p>Student submits proof in Proof Tracker.</p><p>Officer reviews it in Officer Tools.</p><p>Student uses feedback to prepare for mock round and Georgia SLC.</p></div><p className="mt-4 text-sm font-bold leading-6 text-slate-700">Officers should replace these placeholder links with Google Forms, Sheets, and Calendar links owned by the HOSA Gmail or advisor account.</p></Card>
}
