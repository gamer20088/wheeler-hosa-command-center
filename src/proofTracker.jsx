...
function ProofLinkCard({ title, text, button, href, icon: Icon }) {
  return <a href={href} className="flex min-h-36 cursor-pointer flex-col justify-between rounded-2xl bg-white p-4 text-left shadow-sm ring-1 ring-slate-200 transition hover:bg-teal-50 hover:ring-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-300"><div><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-teal-100 text-teal-700"><Icon size={19} /></div><h3 className="text-base font-black text-slate-950">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-700">{text}</p></div><span className="mt-3 inline-flex items-center gap-2 text-sm font-black text-teal-700">{button}<ExternalLink size={14} /></span></a>
}
...
export function ProofTracker({ selectedEvent }) {
  ...
  return <div className="space-y-6"><Card><SectionHeader icon={ClipboardCheck} title="Proof Tracker" subtitle="Student weekly proof submission hub." /><p className="mt-5 max-w-3xl text-sm font-bold leading-6 text-slate-700">Students use this tab to submit weekly evidence that they are preparing for their event.</p><div className="mt-5 grid gap-3 md:grid-cols-3"><ProofLinkCard title="Submit Weekly Proof" button="Open Weekly Proof Form" href={PROOF_LINKS.weeklyProofForm} icon={ClipboardCheck} text="Submit one piece of evidence that you practiced your event this week." /><ProofLinkCard title="Add Weekly Reminder" button="Add Calendar Reminder" href={PROOF_LINKS.weeklyReminder} icon={CalendarCheck} text="Add a weekly reminder so you do not forget proof submissions." /><ProofLinkCard title="View Proof Calendar" button="Open Proof Calendar" href={PROOF_LINKS.proofCalendar} icon={CalendarCheck} text="Check weekly proof deadlines, mock rounds, and SLC prep dates." /></div><p className="mt-4 rounded-xl bg-slate-50 p-3 text-sm font-bold leading-6 text-slate-700 ring-1 ring-slate-200">After you submit proof, officers review it and give feedback through Officer Tools.</p></Card><div className="grid gap-5 lg:grid-cols-2"><Card><ProofList title="What Counts as Proof?" items={proofCounts} icon={CheckCircle2} /></Card><Card><ProofList title="Good Proof for Your Selected Event" items={getProofExamples(selectedEvent)} icon={FileText} /></Card></div><Card className="bg-teal-50 ring-teal-100"><p className="text-xs font-black uppercase tracking-wide text-teal-700">Proof Code</p><p className="mt-1 text-xl font-black text-teal-950">HOSA-PROOF-2026</p><p className="mt-1 text-sm font-bold leading-6 text-teal-950">Use this code on the form so officers know the submission came from Wheeler HOSA.</p></Card><Card className="bg-amber-50 ring-amber-200"><div className="flex gap-3"><AlertTriangle size={20} className="mt-0.5 shrink-0 text-amber-700" /><div><h2 className="text-base font-black text-amber-950">Privacy Warning</h2><div className="mt-2 space-y-2 text-sm font-bold leading-6 text-amber-950"><p>Do not submit medical, personal health, family, or private information. Submit only competition preparation proof.</p><p>Real student submissions should be stored in private Google Forms, Sheets, and Drive folders owned by the HOSA Gmail or advisor account.</p></div></div></div></Card></div>
}

export function ProofOfficerWorkflow() {
  const proofTools = [['Event Interest Form', PROOF_LINKS.eventInterestForm], ['Review Weekly Proof / Open Tracker Sheet', PROOF_LINKS.officerTrackerSheet], ['Officer Feedback Form', PROOF_LINKS.officerFeedbackForm], ['Mock Score Form', PROOF_LINKS.mockRoundScoreForm], ['SLC Readiness Check', PROOF_LINKS.slcReadinessCheck]]
  return <Card><SectionHeader icon={ShieldCheck} title="Officer Tools" subtitle="Officers use this tab to review submissions, give feedback, and track readiness." /><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{proofTools.map(([label, url]) => <a key={label} href={url} className="flex min-h-20 cursor-pointer items-center justify-between gap-3 rounded-2xl bg-slate-50 p-4 text-left font-black text-slate-950 ring-1 ring-slate-200 transition hover:bg-teal-50 hover:ring-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-300"><span>{label}</span><ExternalLink size={15} className="shrink-0 text-teal-700" /></a>)}</div><div className="mt-5 grid gap-3 lg:grid-cols-2"><div className="rounded-xl bg-slate-50 p-3 text-sm font-bold leading-6 text-slate-700 ring-1 ring-slate-200"><p>Student submits proof in Proof Tracker.</p><p>Officer reviews it in Officer Tools.</p><p>Student uses feedback to prepare for mock round and Georgia SLC.</p></div><p className="rounded-xl bg-teal-50 p-3 text-sm font-bold leading-6 text-teal-950 ring-1 ring-teal-100">Officers should replace these placeholder links with Google Forms, Sheets, and Calendar links owned by the HOSA Gmail or advisor account.</p></div></Card>
}
