...
export function CompactMyPlan({ selectedEvent, setTab }) {
  ...
  return <div className="space-y-5"><Card><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-teal-700">My Plan</p><h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950">{safeText(event.name, 'Selected event')}</h2><p className="mt-2 max-w-3xl text-sm font-bold leading-6 text-slate-700">Your weekly dashboard for what to do next, what proof to submit, and how close you are to Georgia SLC-ready.</p></div><div className="flex flex-col gap-2 sm:flex-row"><ActionButton onClick={() => setTab('proof')} icon={ClipboardCheck}>Go to Proof Tracker</ActionButton><ActionButton onClick={copyPlan} icon={Copy} variant="light">Copy Plan</ActionButton><ActionButton onClick={downloadPlan} icon={Download} variant="light">Download Plan</ActionButton></div></div><div className="mt-5 grid gap-3 lg:grid-cols-3"><SummaryTile label="Selected Event">{safeText(event.name, 'Choose an event')}</SummaryTile><SummaryTile label="This Week's Action" className="lg:col-span-2">{safeText(event.firstAction)}</SummaryTile></div><div className="mt-3 grid gap-3 lg:grid-cols-[0.9fr_1.1fr]"><SummaryTile label="Weekly Proof You Should Submit"><BulletList items={proofItems.slice(0, 4)} /></SummaryTile><SummaryTile label="Readiness Progress"><CompactReadiness /><p className="mt-3 text-xs font-bold leading-5 text-slate-500">Proof submission and officer review are verified through Google Forms once links are added.</p></SummaryTile></div><div className="mt-4 rounded-xl bg-amber-50 p-3 text-sm font-bold leading-6 text-amber-950 ring-1 ring-amber-200"><div className="flex gap-2"><AlertTriangle size={16} className="mt-1 shrink-0" /><span>{SLC_WARNING}</span></div></div></Card><div className="grid gap-3"><DetailSection title="Your Event in Plain English"><p className="text-sm leading-6 text-slate-700">{guide.babySummary}</p></DetailSection><DetailSection title="Competition Process"><div className="grid gap-2 text-sm leading-6 text-slate-700"><p><span className="font-black text-slate-900">Process type:</span> {safeText(event.processType)}</p><p><span className="font-black text-slate-900">Round One:</span> {safeText(event.roundOneType)}</p><p><span className="font-black text-slate-900">Round Two:</span> {safeText(event.roundTwoType)}</p><p><span className="font-black text-slate-900">Test required?</span> {yesNo(event.testRequired)}</p><p><span className="font-black text-slate-900">Upload required?</span> {yesNo(event.digitalUpload)}</p><p><span className="font-black text-slate-900">Presentation required?</span> {yesNo(event.presentationRequired)}</p><p><span className="font-black text-slate-900">SLC note:</span> {safeText(event.slcVerificationNote)}</p></div></DetailSection><DetailSection title="Requirements"><BulletList items={guide.requirementsChecklist} /></DetailSection><DetailSection title="Before Georgia SLC"><BulletList items={guide.beforeSlcChecklist} /></DetailSection><DetailSection title="Ready When"><BulletList items={guide.readinessSigns} /></DetailSection><DetailSection title="Weekly Training Plan"><BulletList items={event.weekly} /></DetailSection><DetailSection title="Evidence to Collect"><BulletList items={event.evidence} /></DetailSection><DetailSection title="Common Mistakes"><BulletList items={event.mistakes} /></DetailSection><DetailSection title="Officer Feedback Next Step"><p className="text-sm font-bold leading-6 text-slate-700">After submitting weekly proof, ask an officer to review it and mark your next action before the next meeting.</p></DetailSection></div></div>
}
