...
function FlowButton({ children, onClick, href, icon: Icon }) {
  const className = 'inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2 text-sm font-black text-white transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-300'
...
export function readinessPlanText(event) {
  const processLines = [
  ...
  const readinessLines = READINESS_LEVELS.map(([level, text]) => `${level}: ${text}`).join('\n')
...
export function StartHereSection({ setTab }) {
  return <MiniCard><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-teal-700">Start Here</p><h2 className="mt-1 text-xl font-black tracking-tight text-slate-950 sm:text-2xl">Use the portal in this order</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Move from event selection to weekly proof and mock-round readiness without guessing what to do next.</p></div><div className="flex flex-col gap-2 sm:flex-row"><FlowButton onClick={() => setTab('finder')} icon={Target}>Take Event Finder</FlowButton><FlowButton onClick={() => setTab('proof')} icon={ClipboardCheck}>Go to Proof Tracker</FlowButton></div></div><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{START_STEPS.map(([title, text], index) => <div key={title} className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><div className="mb-2 flex h-8 w-8 items-center justify-center rounded-full bg-teal-100 text-sm font-black text-teal-800">{index + 1}</div><h3 className="text-sm font-black text-slate-950">{title}</h3><p className="mt-1 text-sm leading-6 text-slate-700">{text}</p></div>)}</div></MiniCard>
}

export function ReadinessLevels({ selectedEvent, backupEvent, setTab }) {
  return <MiniCard><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-teal-700">Readiness levels</p><h3 className="mt-1 text-lg font-black text-slate-950">Your path to Georgia SLC-ready</h3><p className="mt-2 text-sm leading-6 text-slate-600">Real proof submission and officer review will be verified through Google Forms once links are added. For now, use this as your weekly checklist.</p></div><FlowButton onClick={() => setTab('proof')} icon={ClipboardCheck}>Go to Proof Tracker</FlowButton></div><div className="mt-5 grid gap-3">{READINESS_LEVELS.map(([level, text], index) => <div key={level} className={`rounded-xl p-3 ring-1 ${index <= 1 ? 'bg-teal-50 ring-teal-200' : 'bg-slate-50 ring-slate-200'}`}><div className="flex gap-3"><CheckCircle2 size={18} className={`mt-0.5 shrink-0 ${index <= 1 ? 'text-teal-600' : 'text-slate-400'}`} /><div><h4 className="text-sm font-black text-slate-950">{level}</h4><p className="mt-1 text-sm leading-6 text-slate-700">{text}</p></div></div></div>)}</div><div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-slate-950 p-4 text-white"><p className="text-xs font-black uppercase tracking-wide text-teal-300">Main event</p><p className="mt-1 text-lg font-black">{safeText(selectedEvent?.name, 'Choose an event')}</p></div><div className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Backup event</p><p className="mt-1 text-lg font-black text-slate-950">{safeText(backupEvent?.name, 'Use Event Finder to compare a backup')}</p></div></div></MiniCard>
}

export function PlanProcessSummary({ event }) {
  return <MiniCard><h3 className="flex items-center gap-2 text-base font-black text-slate-950"><CalendarCheck size={18} className="text-teal-600" />Competition Process</h3><div className="mt-4 grid gap-2 text-sm leading-6 text-slate-700"><p><span className="font-black text-slate-900">Process type:</span> {safeText(event?.processType)}</p><p><span className="font-black text-slate-900">Test required?</span> {yesNo(event?.testRequired)}</p><p><span className="font-black text-slate-900">Upload required?</span> {yesNo(event?.digitalUpload)}</p><p><span className="font-black text-slate-900">Presentation required?</span> {yesNo(event?.presentationRequired)}</p><p><span className="font-black text-slate-900">SLC note:</span> {safeText(event?.slcVerificationNote)}</p></div></MiniCard>
}

export function OfficerFeedbackNextStep({ setTab }) {
  return <MiniCard className="bg-teal-50 ring-teal-100"><h3 className="flex items-center gap-2 text-base font-black text-teal-950"><MessageSquare size={18} className="text-teal-700" />Officer Feedback Next Step</h3><p className="mt-3 text-sm font-bold leading-6 text-teal-950">After submitting weekly proof, ask an officer to review it and mark your next action before the next meeting.</p><div className="mt-4"><FlowButton onClick={() => setTab('proof')} icon={ClipboardCheck}>Go to Proof Tracker</FlowButton></div></MiniCard>
}
