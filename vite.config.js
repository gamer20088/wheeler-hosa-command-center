...
function addFinalEventBatch() {
  return {
    ...
    transform(code, id) {
      ...
      if (normalizedId.endsWith('/src/eventProcess.jsx')) {
        if (code.includes('ProofExamplesList')) return null

        let updated = code.replace(
          /(import \{ StudentFriendlyGuideSection, StudentGuideWarning \} from '.\/babyEventGuide.jsx'\n)/,
          "$1import { ProofExamplesList } from './proofTracker.jsx'\n",
        )
        updated = updated.replace(
          '<ProcessDetails event={event} /><StudentFriendlyGuideSection event={event} /><ListBlock title="This week\'s first action"',
          '<ProcessDetails event={event} /><StudentFriendlyGuideSection event={event} /><ProofExamplesList event={event} /><ListBlock title="This week\'s first action"',
        )
      ...
      updated = updated
        .replace("const NAV = [['home', 'Home', Home], ['finder', 'Event Finder', Target], ['library', 'Event Library', ClipboardList], ['plan', 'My Plan', CalendarCheck], ['guide', 'Plain English Guide', BookOpen], ['officers', 'Officer Tools', ShieldCheck]]", "const NAV = [['home', 'Home', Home], ['finder', 'Event Finder', Target], ['library', 'Event Library', ClipboardList], ['plan', 'My Plan', CalendarCheck], ['proof', 'Proof Tracker', ClipboardCheck], ['guide', 'Plain English Guide', BookOpen], ['officers', 'Officer Tools', ShieldCheck]]")
        ...
        .replace('lg:grid-cols-6', 'lg:grid-cols-7')
        ...
        .replace('const planText = `Wheeler HOSA Event Plan\\n\\nEvent: ${selectedEvent.name}\\nTraining track: ${selectedEvent.track}\\nFormat: ${selectedEvent.format}\\nTeam: ${selectedEvent.team}\\nEstimated weekly time: ${selectedEvent.time}+ hours\\n\\nWhy it fits:\\n${selectedEvent.why}\\n\\nThis week\\\'s action:\\n${selectedEvent.firstAction}\\n\\nWeekly training:\\n${selectedEvent.weekly.map((item, index) => `${index + 1}. ${item}`).join(\'\\n\')}\\n\\nEvidence to collect:\\n${selectedEvent.evidence.map((item, index) => `${index + 1}. ${item}`).join(\'\\n\')}\\n\\nCommon mistakes:\\n${selectedEvent.mistakes.map((item, index) => `${index + 1}. ${item}`).join(\'\\n\')}`', 'const listText = (items) => (Array.isArray(items) ? items : []).map((item, index) => `${index + 1}. ${item}`).join(\'\\n\')\n  const planText = `Wheeler HOSA Event Plan\\n\\nEvent: ${selectedEvent.name}\\nTraining track: ${selectedEvent.track}\\nFormat: ${selectedEvent.format}\\nTeam: ${selectedEvent.team}\\nEstimated weekly time: ${selectedEvent.time}+ hours\\n\\nWhat you are signing up for:\\n${guidePlanText(selectedEvent)}\\n\\nDo this first:\\n${selectedEvent.firstAction}\\n\\nWeekly proof ideas:\\n${proofPlanText(selectedEvent)}\\n\\n${readinessPlanText(selectedEvent)}\\n\\nWeekly training:\\n${listText(selectedEvent.weekly)}\\n\\nEvidence to collect:\\n${listText(selectedEvent.evidence)}\\n\\nCommon mistakes:\\n${listText(selectedEvent.mistakes)}`')
        .replace('function MyPlan({ selectedEvent }) {', 'function MyPlan({ selectedEvent, backupEvent, setTab }) {')
        .replace('<div className="mt-5 flex flex-col gap-2 sm:flex-row"><Button onClick={copyPlan} icon={Copy}>Copy Plan</Button><Button onClick={downloadPlan} variant="dark" icon={Download}>Download Plan</Button></div></Card>', '</Card>')
        .replace('<div className="space-y-5"><Card><ListBlock title="Weekly training plan" items={selectedEvent.weekly} icon={ClipboardCheck} /></Card>', '<div className="space-y-5"><PlainEnglishPlan event={selectedEvent} /><ProofPlanSection event={selectedEvent} /><ReadinessLevels selectedEvent={selectedEvent} backupEvent={backupEvent} setTab={setTab} /><PlanProcessSummary event={selectedEvent} /><OfficerFeedbackNextStep setTab={setTab} /><Card><div className="flex flex-col gap-2 sm:flex-row"><Button onClick={copyPlan} icon={Copy}>Copy Plan</Button><Button onClick={downloadPlan} variant="dark" icon={Download}>Download Plan</Button></div></Card><Card><ListBlock title="Weekly training plan" items={selectedEvent.weekly} icon={ClipboardCheck} /></Card>')
        ...
        .replace('const selectedEvent = EVENTS.find((event) => event.id === selectedEventId) || EVENTS[0]', 'const selectedEvent = EVENTS.find((event) => event.id === selectedEventId) || EVENTS[0]\n  const backupEvent = scoredEvents.find((event) => event.id !== selectedEvent.id) || EVENTS.find((event) => event.id !== selectedEvent.id)')
        .replace('{activeTab === \'home\' && <HomePage setTab={setActiveTab} />}', '{activeTab === \'home\' && <div className="space-y-5"><HomePage setTab={setActiveTab} /><StartHereSection setTab={setActiveTab} /></div>}')
        .replace('{activeTab === \'plan\' && <MyPlan selectedEvent={selectedEvent} />}{activeTab === \'guide\'', '{activeTab === \'plan\' && <MyPlan selectedEvent={selectedEvent} backupEvent={backupEvent} setTab={setActiveTab} />}{activeTab === \'proof\' && <ProofTracker selectedEvent={selectedEvent} />}{activeTab === \'guide\'')
        .replace('</Card><Card className="bg-teal-50 ring-teal-100"><Header icon={LinkIcon} title="Replace later"', '</Card><ProofOfficerWorkflow /><Card className="bg-teal-50 ring-teal-100"><Header icon={LinkIcon} title="Replace later"')
